...
public class DatastoreServer
{
	...
	{	
		...
		try{
			serverCommand=parseRequest(inputStream);
		}catch(IOException e){
			logger.error("Exception while processing request, Server Exception " + e.getMessage());
		}
		catch(Exception e){
			logger.error("Exception while processing request, Server Exception " + e.getMessage());
		}
	...
	private ServerCommand parseRequest(InputStream inputStream) throws ServerException,IOException {
		// TODO Auto-generated method stub
			ServerCommand command=null;
			try {
				String operation=StreamUtil.readLine(inputStream);
				...
				else if(operation.equalsIgnoreCase("directory")){
